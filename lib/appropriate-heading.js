...
function appropriateHeading (tree, file, preferred) {
  ...
  if (!is('heading', firstNonCommentMarkerNode)) {
    ...
    const expected = dirnames.slice(-1).join('').toLowerCase()
    const expectedScoped = dirnames.slice(-2).join('/').toLowerCase()

    const isScoped = expectedScoped.startsWith('@')

    const matchExpected = match(expected, actual, preferred || 'exact')
    const matchExpectedScoped = match(expectedScoped, actual, preferred || 'exact')
